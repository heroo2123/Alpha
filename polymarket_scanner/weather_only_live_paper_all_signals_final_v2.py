...
from .weather_only_live_paper_all_signals_final import FinalAllPaperWeatherLiveService
from .weather_only_live_paper_all_signals_v3 import (
    MAKER_SETTLEMENT_NOTIFY_SENT,
    MAKER_SETTLEMENT_NOTIFY_UNCERTAIN,
)
from .weather_only_live_paper_all_signals_v8 import SOURCE_SHOCK_EVIDENCE, SOURCE_SHOCK_LANE, _excluded
...
from .weather_only_paper_corrective import CorrectiveSettlementEngine, DeliveryUncertain, final_token_payout_v4
...
class FinalAllPaperWeatherLiveServiceV2(FinalAllPaperWeatherLiveService):
    ...
    async def _fresh_wrh_state(self, compiled, *, after_time: float):
        ...
        evidence = build_wrh_observed_extreme_asof(result.snapshot, compiled, as_of=fetched)
    ...
    async def _forecast_post_receipt_execution(self, candidate: dict, event: dict, *, telegram_sent_at: float) -> dict | None:
        execution = await super()._forecast_post_receipt_execution(candidate, event, telegram_sent_at=telegram_sent_at)
    ...
    async def _same_day_exact_recheck(self, candidate: dict, event: dict, *, after_time: float | None):
        checked = await super()._same_day_exact_recheck(candidate, event, after_time=after_time)
        if checked is None:
            return None
        ...
        if after_time is not None:
            ...
            result, evidence = await self._fresh_wrh_state(compiled, after_time=float(after_time))
    ...
    async def _save_and_send_same_day(self, candidate: dict, event: dict) -> tuple[bool, str | None]:
        ...
        fingerprint = _sha({"lane": SAME_DAY_FRIEND_LANE, "event_id": fresh["event_id"], "market_id": fresh["market_id"], "observed_extreme": fresh["observed_extreme"]})
        ...
        try:
            message_id = await self.telegram.send_html(self._same_day_message(fresh), url=_event_link(event), expires_at=float(fresh["decision_expires_at"]))
            ...
            await asyncio.to_thread(self.positions.set_signal_status, signal_id, "EXPIRED" if "EXPIRED" in exc.code else "DELIVERY_FAILED")
            return True, exc.code
        sent_at = time.time()
        ...
        try:
            ...
            return True, code
        if post is None:
            await self._mark_v5_not_actionable(signal_id, fresh, "POST_RECEIPT_SAME_DAY_NO_LONGER_ELIGIBLE")
            return True, None
        post_fresh, post_exact, bucket, params = post
        book = post_exact.books[str(bucket.yes_token)]
        weather_evidence = post_fresh.get("post_receipt_weather_evidence")
        if not isinstance(weather_evidence, dict):
            weather_evidence = {
                key: post_fresh[key]
                for key in (
    ...
    async def _source_shock_exact_recheck(self, candidate: dict, event: dict, *, after_time: float | None, decision_expires_at: float | None):
        checked = await super()._source_shock_exact_recheck(candidate, event, after_time=after_time, decision_expires_at=decision_expires_at)
        if checked is None:
            return None
        fresh, exact, bucket, params = checked
        if after_time is not None:
    ...
    async def _save_and_send_source_shock(self, candidate: dict, event: dict) -> tuple[bool, str | None]:
        try:
            checked = await self._source_shock_exact_recheck(candidate, event, after_time=None, decision_expires_at=None)
        except (StrictWeatherContractError, V4InvariantError, WeatherCLOBError) as exc:
        ...
        if time.time() >= expires_at:
            ...
            return False, None
        from .weather_only_live_paper_v4 import _sha
        decision_id = _sha({"lane": SOURCE_SHOCK_LANE, "event_id": fresh["event_id"], "market_id": fresh["market_id"], "observed_extreme": fresh["new_official_extreme"], "capture_sha256": fresh["capture_sha256"], "book_hash": fresh["book_hash"]})
        fresh.update({"decision_id": decision_id, "decision_expires_at": expires_at, "pre_send_recheck_finished_at": float(exact.finished_at), "paper_execution_protocol_version": PAPER_EXECUTION_PROTOCOL_V5, "paper_target_stake_usd": float(self.paper_stake_usd)})
        fingerprint = _sha({"lane": SOURCE_SHOCK_LANE, "event_id": fresh["event_id"], "market_id": fresh["market_id"], "side": "NO"})
        signal_id = self.positions.save_signal(
            fingerprint=fingerprint, lane=SOURCE_SHOCK_LANE, evidence_class=SOURCE_SHOCK_EVIDENCE,
            event_id=str(fresh["event_id"]), market_id=str(fresh["market_id"]), side="NO",
            token_id=str(fresh["token_id"]), model_probability=None, entry_cost=float(fresh["entry_cost"]),
            raw_gap=None, theoretical_payout=1.0, created_at=float(exact.finished_at), payload=fresh,
        )
        if signal_id is None:
            return False, None
        await asyncio.to_thread(self.positions.set_signal_status, signal_id, "PENDING_DELIVERY")
        try:
            message_id = await self.telegram.send_html(self._source_shock_message(fresh), url=_event_link(event), expires_at=expires_at)
        except DeliveryUncertain as exc:
            await asyncio.to_thread(self.positions.set_signal_status, signal_id, "DELIVERY_UNCERTAIN")
            return True, exc.code
        except WeatherLivePaperError as exc:
        ...
        await asyncio.to_thread(self.positions.set_signal_status, signal_id, "POST_RECEIPT_RECHECK")
        try:
            post = await self._source_shock_exact_recheck(fresh, event, after_time=sent_at, decision_expires_at=expires_at)
        except (StrictWeatherContractError, V4InvariantError, WeatherCLOBError) as exc:
            code = getattr(exc, "code", type(exc).__name__)
            await self._mark_v5_not_actionable(signal_id, fresh, code)
            return True, code
        if post is None:
            await self._mark_v5_not_actionable(signal_id, fresh, "POST_RECEIPT_SOURCE_SHOCK_NO_LONGER_EXECUTABLE")
            return True, None
        post_fresh, post_exact, bucket, params = post
    ...
    async def _progress_maker_orders(self, by_id: dict[str, dict]) -> list[str]:
        ...
        for order in active:
            if order.status not in {RESTING, PARTIALLY_SIMULATED}:
                continue
            try:
                await asyncio.to_thread(self._cancel_maker_order, order, "QUEUE_POSITION_UNCERTIFIED_TRADE_ONLY_STREAM")
            except Exception as exc:
                errors.append(f"MAKER_QUEUE_QUARANTINE:{order.order_id}:{getattr(exc, 'code', type(exc).__name__)}")
        return errors
    ...
    async def _settle_maker_orders(self) -> list[str]:
        ...
        for result in results:
            ...
            if not isinstance(market, dict):
                continue
            try:
                payout = final_token_payout_v4(order.token_id, market, expected_condition_id=order.condition_id, expected_side=str(order.outcome).upper())
            except Exception as exc:
                errors.append(f"MAKER_SETTLEMENT_FINALITY:{order.order_id}:{getattr(exc, 'code', type(exc).__name__)}")
                continue
            if payout is None:
                continue
            evidence = {"source": "GAMMA_RESOLVED_SETTLED_STRICT_TOKEN_PAYOUT_V4", "market_id": order.market_id, "condition_id": order.condition_id, "token_id": order.token_id, "payout": float(payout), "checked_at": time.time(), "uma_finality_required": True, "financial_authority": False}
            try:
                await asyncio.to_thread(self.maker_store.record_settlement, order, payout_per_share=float(payout), evidence=evidence)
                self._maker_settlements_recorded += 1
            except Exception as exc:
                errors.append(f"MAKER_SETTLEMENT:{order.order_id}:{getattr(exc, 'code', type(exc).__name__)}")
        pending = await asyncio.to_thread(self._pending_maker_settlement_notifications)
        for order_id, payload in pending:
            try:
                message_id = await self.telegram.send_html(self._maker_settlement_message(payload))
            except DeliveryUncertain as exc:
                await asyncio.to_thread(self.maker_store.append_research_event, order_id, event_type=MAKER_SETTLEMENT_NOTIFY_UNCERTAIN, payload={"error": exc.code, "financial_authority": False})
                continue
            except WeatherLivePaperError as exc:
                errors.append(f"MAKER_SETTLEMENT_TELEGRAM:{order_id}:{exc.code}")
                continue
            await asyncio.to_thread(self.maker_store.append_research_event, order_id, event_type=MAKER_SETTLEMENT_NOTIFY_SENT, payload={"telegram_message_id": int(message_id), "financial_authority": False})
        return errors
...
async def _main(args) -> int:
    service = FinalAllPaperWeatherLiveServiceV2(
        db_path=args.db, status_path=args.status, release_file=args.release_file,
        interval_seconds=args.interval_seconds, forecast_cache_seconds=args.forecast_cache_seconds,
        forecast_raw_gap_min=args.forecast_raw_gap_min, max_forecast_events=args.max_forecast_events,
