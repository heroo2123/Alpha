...
V3 deliberately performs local SQLite work in ``asyncio.to_thread`` so database fsync
cannot stall the weather event loop.  The base maker store originally used SQLite's
same-thread default, which is correct for synchronous callers but incompatible with
that worker-thread boundary.  This subclass opens its one connection with
``check_same_thread=False`` and protects every public database operation with one
process-local ``RLock``.  The connection is therefore never used concurrently even
when successive calls land on different executor workers. SQLite transactions remain
the cross-process serialization boundary.
"""
...
MAKER_PAPER_ACCOUNTING_VERSION = "weather_maker_paper_accounting_v2_serialized_cross_thread_sqlite"
...
class MakerPaperAccountingStore(WeatherMakerShadowStore):
    """Thread-handoff-safe facade with one serialized SQLite connection."""
    ...
    def save_new_order(
        self, order: VirtualMakerOrder, *, recorded_at: float | None = None
    ) -> VirtualMakerOrder:
        with self._db_lock:
            return super().save_new_order(order, recorded_at=recorded_at)

    def load_order(self, order_id: str) -> VirtualMakerOrder:
        with self._db_lock:
            return super().load_order(order_id)
    ...
    def append_research_event(
        ...
        *,
        ...
        with self._db_lock:
            return super().append_research_event(
                order_id,
                event_type=event_type,
                payload=payload,
                recorded_at=recorded_at,
    ...
    def audit_order(self, order_id: str) -> dict:
        with self._db_lock:
            return super().audit_order(order_id)
        ...
        *,
        ...
        with self._db_lock:
            ...
            if self._has_event(order.order_id, MAKER_SETTLEMENT_EVENT):
                ...
                if existing is None:
                    raise MakerPaperAccountingError(
                        "MAKER_ACCOUNTING_SETTLEMENT_EVENT_INVALID"
                    )
                return existing
