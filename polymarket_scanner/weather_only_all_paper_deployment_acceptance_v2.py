...
ALL_PAPER_DEPLOYMENT_ACCEPTANCE_V2_VERSION = (
    "weather_all_paper_first_cycle_acceptance_v14_v8_history_network_global_recall"
)
...
def _true(status: dict, key: str, code: str) -> None:
    if status.get(key) is not True: raise AllPaperDeploymentAcceptanceError(code)

def _false(status: dict, key: str, code: str) -> None:
    if status.get(key) is not False: raise AllPaperDeploymentAcceptanceError(code)

def accept_first_all_paper_cycle_v2(status: object, *, expected_release_sha: str, not_before: float, now: float | None = None, max_age_seconds: float = 600.0) -> AllPaperFirstCycleAcceptanceV2:
    base = accept_first_all_paper_cycle(status, expected_release_sha=expected_release_sha, not_before=not_before, now=now, max_age_seconds=max_age_seconds)
    if not isinstance(status, dict): raise AllPaperDeploymentAcceptanceError("ALL_PAPER_STATUS_TYPE_INVALID")
    for key, expected, code in (
        ...
        if status.get(key) != expected: raise AllPaperDeploymentAcceptanceError(code)
    for key, code in (
    ...
    if not isinstance(recall, dict) or recall.get("complete") is not True or int(recall.get("pages") or 0) <= 0 or int(recall.get("scanned_events") or 0) <= 0:
    ...
    if not isinstance(sync, dict) or sync.get("healthy") is not True: raise AllPaperDeploymentAcceptanceError("ALL_PAPER_OPERATOR_SYNC_SUMMARY_UNHEALTHY")
    if int(sync.get("unconfirmed") or 0) != 0: raise AllPaperDeploymentAcceptanceError("ALL_PAPER_OPERATOR_SYNC_UNCONFIRMED")
    if int(sync.get("failed") or 0) != 0: raise AllPaperDeploymentAcceptanceError("ALL_PAPER_OPERATOR_SYNC_FAILED")
    if list(status.get("operator_message_sync_errors") or []): raise AllPaperDeploymentAcceptanceError("ALL_PAPER_OPERATOR_SYNC_ERRORS_PRESENT")
    if int(status.get("operator_retry_max_per_evidence") or 0) != 3: raise AllPaperDeploymentAcceptanceError("ALL_PAPER_OPERATOR_RETRY_BOUND_MISMATCH")
    if float(status.get("operator_retry_cooldown_seconds") or 0.0) < 60.0: raise AllPaperDeploymentAcceptanceError("ALL_PAPER_OPERATOR_RETRY_COOLDOWN_TOO_SHORT")
    if status.get("isolated_settings_overrides") != ["telegram_bot_token", "telegram_chat_id"]: raise AllPaperDeploymentAcceptanceError("ALL_PAPER_ISOLATED_SETTINGS_OVERRIDE_SET_MISMATCH")
