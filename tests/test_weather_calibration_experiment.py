...
from polymarket_scanner.weather_calibration_policy import (
    WEATHER_GEFS_CALIBRATION_POLICY_ID,
...
def test_experiment_manifest_binds_collection_and_frozen_policy_without_money_authority():
    policy = frozen_weather_calibration_policy()
    ...
    assert manifest.statistical_policy_id == WEATHER_GEFS_CALIBRATION_POLICY_ID
    assert manifest.statistical_policy_sha256 == policy.policy_sha256
    assert len(manifest.statistical_policy_sha256) == 64
