...
from polymarket_scanner.weather_only_independent_review_corrective import INDEPENDENT_REVIEW_CORRECTIVE_VERSION
from polymarket_scanner.weather_only_independent_review_corrective_v2 import INDEPENDENT_REVIEW_CORRECTIVE_V2_VERSION
from polymarket_scanner.weather_only_independent_review_corrective_v3 import INDEPENDENT_REVIEW_CORRECTIVE_V3_VERSION
...
from polymarket_scanner.weather_only_live_paper_all_signals_final_v4 import FINAL_ALL_PAPER_RUNTIME_V4_VERSION
...
def test_final_all_paper_first_cycle_accepts_only_complete_safe_profile():
    ...
    assert accepted.runtime_version == FINAL_ALL_PAPER_RUNTIME_V4_VERSION
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("final_all_paper_runtime_v4_version", None, "ALL_PAPER_FINAL_V4_WRAPPER_VERSION_MISMATCH"),
        ("independent_review_corrective_v3_version", None, "ALL_PAPER_INDEPENDENT_REVIEW_CORRECTIVE_V3_MISSING"),
        ("post_receipt_execution_required", False, "ALL_PAPER_POST_RECEIPT_EXECUTION_NOT_REQUIRED"),
        ("post_receipt_future_day_provider_refetch_required", False, "ALL_PAPER_FUTURE_DAY_PROVIDER_REFETCH_NOT_REQUIRED"),
        ("post_receipt_three_layer_thesis_revalidation_required", False, "ALL_PAPER_THREE_LAYER_POST_RECEIPT_REVALIDATION_NOT_REQUIRED"),
        ("post_receipt_weather_before_clob_required", False, "ALL_PAPER_WEATHER_BEFORE_CLOB_NOT_REQUIRED"),
        ("post_receipt_weather_evidence_durable", False, "ALL_PAPER_WEATHER_EVIDENCE_NOT_DURABLE"),
        ("post_receipt_weather_evidence_identity_bound", False, "ALL_PAPER_WEATHER_EVIDENCE_NOT_IDENTITY_BOUND"),
...
def test_all_paper_first_cycle_fails_closed_on_safety_or_strategy_drift(key, value, code):
    ...
    with pytest.raises(AllPaperDeploymentAcceptanceError) as exc:
        accept_first_all_paper_cycle(status, expected_release_sha=SHA, not_before=NOW - 10.0, now=NOW)
    assert exc.value.code == code


def test_all_paper_first_cycle_rejects_post_receipt_layer_set_drift():
    status = _status()
    status["same_day_post_receipt_layers"] = ["WRH", "GEFS31"]
    with pytest.raises(AllPaperDeploymentAcceptanceError) as exc:
        accept_first_all_paper_cycle(status, expected_release_sha=SHA, not_before=NOW - 10.0, now=NOW)
    assert exc.value.code == "ALL_PAPER_POST_RECEIPT_LAYER_SET_MISMATCH"
...
def test_all_paper_first_cycle_rejects_stale_or_wrong_release():
    with pytest.raises(AllPaperDeploymentAcceptanceError) as exc:
        accept_first_all_paper_cycle(_status(), expected_release_sha="b" * 40, not_before=NOW - 10.0, now=NOW)
    assert exc.value.code == "ALL_PAPER_STATUS_RELEASE_MISMATCH"
    status = _status()
    status["finished_at"] = NOW - 1000.0
    with pytest.raises(AllPaperDeploymentAcceptanceError) as exc:
        accept_first_all_paper_cycle(
            status, expected_release_sha=SHA, not_before=NOW - 2000.0, now=NOW, max_age_seconds=600.0
        )
