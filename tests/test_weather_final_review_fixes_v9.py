...
class _BacklogStore:
    ...
    def pending_operator_sync(self,limit:int=50): return [self.rows[k] for k in sorted(self.rows)[:limit]]
...
def test_root_custody_and_final_v9_are_wired_into_immutable_generation_authority():
    install=(ROOT/"deploy/install-weather-paper-host-trust.sh").read_text(); snapshot=(ROOT/"deploy/weather-paper-host-snapshot.sh").read_text(); wrapper=(ROOT/"deploy/snapshot-all-paper-rollback.sh").read_text(); recovery=(ROOT/"deploy/weather-paper-host-recovery.sh").read_text(); prepare=(ROOT/"deploy/prepare-all-paper-candidate-v3.sh").read_text(); renderer=(ROOT/"deploy/render-all-paper-unit.py").read_text(); attester=(ROOT/"deploy/attest-all-paper-runtime-v2.py").read_text()
    assert 'ROLLBACK_DIR="/var/lib/polymarket-weather-paper-rollback"' in install and 'host authority already installed; bootstrap refuses replacement' in install
    assert 'GENERATIONS="${ROLLBACK_DIR}/generations"' in snapshot and 'weather-paper-cutover-generation-v1' in snapshot and '[[ "${PREDECESSOR_SHA}" != "${CANDIDATE_SHA}" ]]' in snapshot
    assert "snapshot-generation-v4" not in snapshot and "rollback-manifest-v4.json" not in snapshot
    assert "exec sudo /usr/bin/env -i" in wrapper and "HOME=/nonexistent" in wrapper
    assert 'GEN="${ROLLBACK_DIR}/generations/${GENERATION_ID}"' in recovery and 'checkout --detach "${PREDECESSOR_SHA}"' in recovery and 'predecessor-venv.tar' in recovery
    assert 'HOST_SNAPSHOT="${LIBEXEC}/snapshot-rollback.sh"' in prepare and prepare.index('"${HOST_SNAPSHOT}" --candidate-sha') < prepare.index('checkout --detach "${RELEASE_SHA}"')
    assert '.releases/${RELEASE_SHA}' in prepare and "weather_only_live_paper_all_signals_final_v9" in prepare and "weather_only_live_paper_all_signals_final_v9" in renderer and "weather_only_live_paper_all_signals_final_v9" in attester
