...
# Enable V8 boot persistence only after active exact-SHA acceptance and fresh source proof.
...
[[ -f "${START_EPOCH_FILE}" && ! -L "${START_EPOCH_FILE}" ]] || fail "V8 start boundary missing/invalid"
...
systemctl is-active --quiet "${UNIT}" 2>/dev/null || fail "accepted V8 service is not active"
...
except Exception: raise SystemExit("invalid V8 start boundary")
if not math.isfinite(value) or value <= 0: raise SystemExit("invalid V8 start boundary")
PY
...
systemctl is-enabled --quiet "${UNIT}" || fail "V8 service was not enabled"
systemctl is-active --quiet "${UNIT}" || fail "V8 service stopped while enabling persistence"
...
trap - EXIT
printf '\nPASS: V8 all-weather PAPER bot is enabled for restart persistence.\n'
printf 'A durable post-start WRH+NWS+GEFS capture and exact V8 runtime were proven first.\n'
