...
def render(app_dir: Path, config_dir: Path, user: str, release_sha: str = "0" * 40) -> str:
    for value in (str(app_dir), str(config_dir), user, release_sha):
        if not re.fullmatch(r"[/A-Za-z0-9_.-]+", value):
            raise ValueError("service paths/user/SHA must be absolute, whitespace-free safe names")
    ...
    generation_file = f"{config_dir}/weather-paper-cutover-generation.id"
    host_gate = f"/usr/bin/python3 {HOST_RELEASE_GATE} verify-checkout --app-dir {app_dir} --release-file {release_file} --generation-file {generation_file}"
    unset = " ".join(FORBIDDEN_ENV_NAMES)
...
ExecStart={python} -E -s -m {ALL_PAPER_MODULE} --db {state}/weather-paper.sqlite --status {state}/status.json --release-file {release_file} --interval-seconds 180 --forecast-cache-seconds 900 --forecast-raw-gap-min 0.08 --max-forecast-events 6 --paper-stake-usd 10
Restart=on-failure
RestartSec=15
TimeoutStopSec=20
MemoryHigh=280M
