...
for snapshot in previous-release.sha previous-tree.sha "${UNIT}" previous-active previous-enabled previous-db-present previous-db.sha256 previous-venv.tar previous-venv.json previous-venv-release.sha snapshot-generation-v3; do
  [[ -f "${ROLLBACK_DIR}/${snapshot}" ]] || fail "missing host rollback snapshot: ${snapshot}"
done
...
trap - EXIT
printf 'PASS: host-approved V8 candidate active and accepted; boot persistence remains disabled.\n'
