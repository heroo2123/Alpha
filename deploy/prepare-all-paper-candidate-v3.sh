...
SOURCE_REF="${ALPHA_WEATHER_SOURCE_REF:-${2:-weather-all-paper-adversarial-fixes-2026-09-16}}"
RELEASE_SHA="${1:-}"
...
for host_tool in "${GATE}" "${HOST_VENV}" "${HOST_RECOVERY}"; do [[ -f "${host_tool}" ]] || fail "host trust bootstrap missing: ${host_tool}"; done
...
for required in previous-release.sha previous-tree.sha "${UNIT}" previous-active previous-enabled \
  ...
  [[ -f "${ROLLBACK_DIR}/${required}" ]] || fail "missing host rollback snapshot: ${required}"
done
[[ "$(tr -d '[:space:]' < "${ROLLBACK_DIR}/snapshot-generation-v3")" == 'all-paper-rollback-v3-host-authority' ]] || fail "rollback generation not host-authority v3"
...
for required in deploy/render-all-paper-unit.py deploy/attest-all-paper-runtime-v2.py \
  ...
  polymarket_scanner/weather_only_live_paper_all_signals_final_v8.py \
...
grep -qF "${FINAL_MODULE}" "${APP_DIR}/deploy/render-all-paper-unit.py" || fail "renderer is not final-v8"
...
print('Final V8 configuration/network import guards passed.')
PY
...
PREPARE_MUTATED=0
trap - EXIT
printf 'PASS: host-approved V8 candidate prepared, stopped and disabled. Release=%s\n' "${RELEASE_SHA,,}"
