...
def render(app_dir: Path, config_dir: Path, user: str) -> str:
    ...
    return f"""[Unit]
Description=Polymarket weather-only LIVE PAPER guarded tracked signals
...
ExecStart={python} -m polymarket_scanner.weather_only_live_paper_v3 --db {state}/weather-paper.sqlite --status {state}/status.json --release-file {config_dir}/release.sha --interval-seconds 180 --forecast-cache-seconds 900 --forecast-raw-gap-min 0.08 --max-forecast-events 6 --paper-stake-usd 10
Restart=on-failure
RestartSec=15
TimeoutStopSec=20
MemoryHigh=280M
