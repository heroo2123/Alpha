...
Repository implementation and target-host acceptance are separate gates. This guide describes the composed production path being verified on the integration branch. Do not deploy from an unverified working tree or the historical checkpoint PR.
...
Examples are in `config/production/`. The execution example deliberately contains null account/risk values: there is no invented bankroll, loss budget or allocation. Every financial limit must be chosen by the operator. Changing the configuration file immediately removes a running worker's opening authority as well as invalidating the activation digest. Example raw model-gap thresholds are heuristics, not calibrated probabilities or profitability estimates.
...
Create an isolated execution environment from `requirements-execution-hashed.txt`. Set all mandatory account/risk settings, a suitable HTTPS Polygon JSON-RPC endpoint with finalized-block support, private credentials and the independent host policy. The credential schema is defined by `ExchangeEOA.from_credentials_file`; never copy real values into a report or test.
...
V2 has no signed per-order fee cap. The adapter checks current explicit fee data and a positive on-chain maximum fee bound immediately before submission. A zero/unbounded bound is rejected. An administrator can change that bound after signing, so this is not a cryptographic guarantee against all platform fee changes. Actual fee breaches stop new openings and remain visible. Missing/unbounded fee evidence can legitimately make a specific market ineligible.
