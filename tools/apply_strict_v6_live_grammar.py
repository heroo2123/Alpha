...
PATH = Path("polymarket_scanner/weather_only_contract_strict.py")
...
question_start = text.index("def _question_supported(")
rule_start = text.index("def _supported_nws_rule_structure(", question_start)
question_block = r'''_CURRENT_TITLE_RE = re.compile(
...
'''
text = text[:question_start] + question_block + text[rule_start:]

rule_start = text.index("def _supported_nws_rule_structure(")
coherent_start = text.index("def _coherent_rule_identity(", rule_start)
rule_block = r'''def _supported_nws_rule_structure(operative_rules: str, compiled) -> bool:
...
'''
text = text[:rule_start] + rule_block + text[coherent_start:]

compile_start = text.index("def compile_strict_temperature_event(")
compile_block = r'''def compile_strict_temperature_event(event: dict):
    ...
    return compiled
'''
text = text[:compile_start] + compile_block
